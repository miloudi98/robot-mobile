...
static esp_err_t project_handler(httpd_req_t *req) {
...
const char* page = R"rawliteral(
    ...
    <html>
      <body>
        <img src="http://192.168.2.53:81/stream" />
        ...
        <script>
          async function forward() {
            try {
              const res = await fetch('http://192.168.2.53/project/goforward', { method: 'POST', });
              if (!res.ok){
          ...
          async function backward() {
            try {
              const res = await fetch('http://192.168.2.53/project/gobackward', { method: 'POST', });
              if (!res.ok){
                throw new Error('backward failed');
              }
            } catch (error) {
              console.log(error);
            }
          ...
          async function left() {
            try {
              const res = await fetch('http://192.168.2.53/project/goleft', { method: 'POST', });
              if (!res.ok){
          ...
          async function right() {
            try {
              const res = await fetch('http://192.168.2.53/project/goright', { method: 'POST', });
              if (!res.ok){
...
static esp_err_t goforward_handler(httpd_req_t *req) {
  Serial.print('F');

  const char* json_res = "{}";
  httpd_resp_set_type(req, "application/json");
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  return httpd_resp_send(req, json_res, strlen(json_res));
  
}
static esp_err_t gobackward_handler(httpd_req_t *req) {
  Serial.print('B');
    const char* json_res = "{}";
  httpd_resp_set_type(req, "application/json");
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
...
void startCameraServer() {
  ...
  httpd_uri_t forward_uri = {
    .uri = "/project/goforward",
    .method = HTTP_POST,
    .handler = goforward_handler
  };

  httpd_uri_t backward_uri = {
    .uri = "/project/gobackward",
    .method = HTTP_POST,
    .handler = gobackward_handler
  };
    httpd_uri_t left_uri = {
    .uri = "/project/goleft",
    .method = HTTP_POST,
    .handler = goleft_handler
  };
    httpd_uri_t right_uri = {
    .uri = "/project/goright",
    .method = HTTP_POST,
    .handler = goright_handler
  };
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &project_uri);
